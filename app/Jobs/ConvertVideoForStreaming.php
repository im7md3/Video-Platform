...
class ConvertVideoForStreaming implements ShouldQueue
{
    ...
    protected function convertVideo($loopNumber) {
        ...
        for ($i = $loopNumber; $i < 5; $i++) {
            $converted_video->{ 'mp4_Format_'. $this->videoHeight[$i] } = 'media/'.$this->names[$i][0];
            $converted_video->{ 'webm_Format_'. $this->videoHeight[$i] } = 'media/'.$this->names[$i][1];
        }
    ...
    public function handle()
    {
        ...
        $video1 = $ffprobe->streams(public_path('/media//' .$this->video->video_path))->videos()->first();
        ...
        $user->notify(new VideoSuccess($user,$this->video));
